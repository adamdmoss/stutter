...
UTIL_time_t UTIL_getCPUTime(void)
{
    /* time must be initialized, othersize it may fail msan test.
     * No good reason, likely a limitation of timespec_get() for some target */
    UTIL_time_t time = UTIL_TIME_INITIALIZER;
...
/* returns time span in microseconds */
PTime UTIL_clockCPUSpanMicro(UTIL_time_t clockStart )
{
    UTIL_time_t const clockEnd = UTIL_getCPUTime();
    return UTIL_getSpanTimeMicro(clockStart, clockEnd);
}

/* returns time span in nanoseconds */
PTime UTIL_clockCPUSpanNano(UTIL_time_t clockStart )
{
    UTIL_time_t const clockEnd = UTIL_getCPUTime();
    return UTIL_getSpanTimeNano(clockStart, clockEnd);
}
...
void UTIL_waitForCPUNextTick(void)
{
    UTIL_time_t const clockStart = UTIL_getCPUTime();
    UTIL_time_t clockEnd;
    do {
        clockEnd = UTIL_getCPUTime();
    } while (UTIL_getSpanTimeNano(clockStart, clockEnd) == 0);
}
